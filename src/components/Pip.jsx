    ...
    <div className="column">
      <div className="box has-text-centered">
        ...
        <p className="title subtitle is-size-4">
          {t("daistats.token_price", { token: props.token })}
        </p>
        {ilk.priceNxt > 0 && (
          <p className="subtitle is-size-6">
            {t("daistats.next_osm_price")}:{" "}
            <span
              className={nextPrice(ilk.price, ilk.priceNxt)}
              title={ilk.priceNxt}
            >
        ...
        {ilk.zzz &&
          Number(ilk.price).toFixed(4) !== Number(ilk.priceNxt).toFixed(4) && (
            <p className="title subtitle is-size-6">
              Next Possible Update: {ilk.zzz}
            </p>
          )}
        {ilk.priceMedian && (
